...
To produce a new release:
 ...
 2. Run `cargo build --all-features --release` to update `Cargo.lock`
 3. Commit `Cargo.lock` and `CHANGELOG.md` with the message `0.N.0` (just the version number, nothing else)
